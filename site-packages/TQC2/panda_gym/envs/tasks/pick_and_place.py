...
class PickAndPlace(Task):
    ...
    def _create_scene(self):
        ...
        for i in range(self.num_objects):
            ...
            self.sim.create_box(
                ...
                half_extents=self.goal_sizes[i],
                mass=0.0,
                ghost=True,
    ...
    def is_success(self, achieved_goal: np.ndarray, desired_goal: np.ndarray) -> np.ndarray:
        ...
        return np.array(d < self.distance_threshold * 4, dtype=bool)
    
    # def compute_reward(self, achieved_goal, desired_goal, info: Dict[str, Any] = ...)-> np.ndarray:
    #     d = distance(achieved_goal, desired_goal)
    #     if self.reward_type == "sparse":
    #         # print("sparse", -np.array((d > self.distance_threshold), dtype=np.float32))
    #         return -np.array((d > self.distance_threshold), dtype=np.float32)
    #     else:
    #         # print("dense ",-d.astype(np.float32))
    #         return -d.astype(np.float32)
    def compute_reward(self, achieved_goal, desired_goal, info: Dict[str, Any] = ...)-> np.ndarray:
        # achieved_goal과 desired_goal 사이의 거리 계산
        d = distance(achieved_goal, desired_goal)
        # 로그 보상 계산: 거리에 1을 더하고 로그를 취한 후, 음수를 취함
        log_reward = -np.log(d + 1)

        # 보상은 항상 음수 또는 0이어야 함
        # 거리가 0일 때 (목표에 정확히 위치했을 때), 로그 보상은 0이 되어야 함
        # 거리가 증가함에 따라 보상은 음의 무한대로 접근
        return log_reward
